...
import { applyInitialTheme, getThemePreference } from "./services/themeService";
...
function App() {
  ...
  const [isMeLoading, setIsMeLoading] = useState(
    () => isLoggedIn && me == null,
  );
    ...
    await Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          console.warn(
            "API logout failed, continuing to clear local state",
            error,
          );
        } finally {
          localStorage.clear();
    ...
    <div className="flex min-h-screen flex-col bg-surface dark:bg-secondary-dark font-display antialiased">
      <main className="flex-1">
        <Routes>
          ...
          />
          <Route path="/main" element={requireLogin(<MainPage />)} />

          <Route
            path="/me"
            element={requireHydratedUser(
              me?.role !== "admin" ? (
                <ProfilePage />
              ) : (
          ...
          />
          <Route path="/cart" element={requireLogin(<CheckoutPage />)} />
          <Route path="/admin" element={requireAdmin(<AdminPage />)} />
          <Route
            path="/admin/orders"
            element={requireAdmin(<AdminOrdersPage />)}
          />
          <Route path="/admin/pos" element={requireAdmin(<AdminPosPage />)} />
          <Route
            path="/orderstatus"
            element={requireHydratedUser(
              me?.role === "admin" ? <AdminOrdersPage /> : <PostPaymentPage />,
            )}
          />
          <Route
            path="*"
