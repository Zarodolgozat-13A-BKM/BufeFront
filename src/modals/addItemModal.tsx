...
const AddItemModalContent = ({
	...
	onUpdateQuantity,
	qty,
}: AddItemModalProps & { item: ItemModel }) => {
	const dispatch = useAppDispatch();
	const [quantity, setQuantity] = useState(qty > 0 ? qty : 1);
const handleAddToCart = () => { if (qty === 0) { dispatch(addItemToCart({ item, quantity })); } else { const delta = quantity - qty; if (delta !== 0) { onUpdateQuantity(item, delta); } } onClose(); };
