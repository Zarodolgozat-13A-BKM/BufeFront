...
const getOrderBaseDate = (order: OrderModel): Date | null => {
  const sourceDate = order.delivery_date ?? order.created_at;
...
const getSlaTone = (elapsedMinutes: number): "ok" | "warn" | "late" => {
  ...
  if (elapsedMinutes >= 0) return "warn";
  return "ok";
};
...
export const OrderComponent = ({
  ...
  return (
    <div id={`admin-order-${order.id}`} className={"bg-white dark:bg-zinc-800 rounded-xl p-4 md:p-5 border flex flex-col transition-all " + (highlighted ? "border-primary ring-2 ring-primary/40 shadow-lg shadow-primary/10 animate-pulse" : "border-[#e6e0db] dark:border-zinc-700 ") }>
      <div className="flex justify-between items-start mb-4">
        <div>
          <span className="px-3 py-1 bg-primary/10 border border-primary/20 text-[10px] font-semibold rounded-full text-primary tracking-wider uppercase">{new Date(order.delivery_date!).toLocaleTimeString()}</span>
        ...
        <div className="mt-1 flex flex-col items-end gap-1">
          ...
          {elapsedMinutes != null && (
            ...
            >
              {elapsedMinutes < 0 
                ? `${Math.abs(elapsedMinutes)} perc van hátra` 
                : elapsedMinutes === 0 
                  ? "Most esedékes" 
                  : `${elapsedMinutes} perc késés`}
            </span>
          )}
