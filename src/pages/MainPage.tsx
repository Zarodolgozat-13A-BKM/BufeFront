...
const MainPage = () => {
  ...
  const scrollToCategory = useCallback(
    (categoryId: number | null) => {
      ...
      window.requestAnimationFrame(() => {
        ...
        const topOffset =
          headerHeight || stickyHeaderRef.current?.offsetHeight || 0;
        ...
        scrollContainer.scrollTo({
          top: Math.max(0, containerScrollTop),
          behavior: "smooth",
        });
  ...
  const handleFeaturedClickCapture = (
    ...
    <div className="mx-auto max-w-[90%] bg-surface dark:bg-zinc-900 font-display antialiased ">
      ...
      >
        <div className=" fixed top-0 left-0 right-0 z-50">
          <div
            ref={stickyHeaderRef}
            className="w-full bg-surface dark:bg-zinc-900 border-b border-x border-[#e6e0db] dark:border-zinc-800"
          >
            <TopAppBar
              username={
                me?.role !== "admin"
                  ? (me?.full_name ?? "Guest")
                  : "Kattints ide az adminfelület megnyitásához"
              }
            ...
            <div data-cy="mainpage-content">
              ...
              {hasSearchQuery && !hasAnySearchResults ? (
                <div className="px-4 pt-8">
                  <div
                    data-cy="main-no-results"
                    className="mx-auto max-w-2xl rounded-2xl border border-[#e6e0db] bg-surface dark:bg-zinc-900"
                  >
                    <span className="material-symbols-outlined text-3xl text-muted dark:text-zinc-400">
                      search_off
                    </span>
                    <h3 className="mt-3 text-base sm:text-lg lg:text-xl font-bold text-foreground dark:text-white">
                      Nincs találat erre: "{searchQuery.trim()}"
                    </h3>
                    <p className="mt-2 text-sm text-muted dark:text-zinc-400">
                      Próbálj rövidebb keresést, vagy válassz egy ajánlott
                      kategóriát.
                    </p>
              ...
              {filteredCategories.map(({ category, filteredItems }) => {
                ...
                return (
                  <div
                    ...
                    data-cy={`main-category-section-${category.id}`}
                    className="flex flex-col gap-3 lg:gap-4 2xl:gap-6 px-4 lg:px-6 2xl:px-8 pt-6 scroll-mt-32"
                  >
                    <h3
                      data-cy={`main-category-name-${category.id}`}
                      className="text-foreground dark:text-white text-base sm:text-lg lg:text-xl 2xl:text-2xl font-bold leading-tight flex items-center gap-2 mt-5"
                    >
        ...
        <AddItemModal
          ...
          item={selectedItem}
          onUpdateQuantity={updateQuantity}
          qty={selectedItem ? (itemQuantityById[selectedItem.id] ?? 0) : 0}
        />
      </div>
    </div>
  );
