...
import { AdjustInventory, DeleteItem, ToggleActive, ToggleFeatured } from '../services/ItemService'
...
const sortOrdersByField = (orders: OrderModel[], field: SortableOrderField, dir: SortDir) => {
...
const getSortIcon = (field: string, activeField: string, dir: SortDir): ReactNode => {
...
const AdminPage = () => {
  ...
  const handleItemStatusToggle = async (
    ...
    const updated = categories.map((category) => ({
      ...
      items: category.items.map((item) => {
        ...
        if (field === 'is_active') return { ...item, is_active: !item.is_active }
  ...
  const handleItemCreated = async (item: ItemModel) => {
    const updatedCategories = categories.map((category) => ({
      ...category,
      items: category.items.filter((existingItem) => existingItem.id !== item.id),
    }))

    const targetCategoryIndex = updatedCategories.findIndex((category) => category.id === item.category_id)
  ...
  const handleCatDelete = async (cat: CategoryModel) => {
    if (confirm(`Biztosan törölni szeretnéd a "${cat.name}" kategóriát? Ez a művelet nem visszavonható, és minden termék, ami csak ehhez a kategóriához tartozik, szintén törlésre kerül!`)) {
      await DeleteCategory(cat.id.toString())
      dispatch(setCategories(categories.filter((category) => category.id !== cat.id)))
    }
  };
  ...
  const handleCatCreated = async (category: CategoryModel) => {
    ...
    const exists = categories.some((existingCategory) => existingCategory.id === normalizedCategory.id)
      ...
      ? categories.map((existingCategory) =>
        existingCategory.id !== normalizedCategory.id
          ...
          : {
            ...existingCategory,
            ...normalizedCategory,
            items: normalizedCategory.items.length > 0 ? normalizedCategory.items : existingCategory.items,
          },
      )
        ...
        <div className="p-4 md:p-6 space-y-5">
          ...
          {CategoryTableVisible && (
            <div className="w-full xl:w-full rounded-xl border border-[#e6e0db] bg-surface dark:bg-zinc-800/50 dark:border-zinc-800 p-4 md:p-5">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <h2 className="text-xl font-bold text-foreground dark:text-white">Kategóriák ({categories.length})</h2>
                <button
                  onClick={() => {
          ...
          {ItemTableVisible && (
            <div className="min-w-0 w-full flex-1 rounded-xl border border-[#e6e0db] bg-surface dark:bg-zinc-800/50 dark:border-zinc-800 p-4 md:p-5">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <h2 className="text-xl font-bold text-foreground dark:text-white">Termékek ({items.length})</h2>
          ...
          {orderTableVisible && (
            <div className="min-w-0 w-full flex-1 rounded-xl border border-[#e6e0db] bg-surface dark:bg-zinc-800/50 dark:border-zinc-800 p-4 md:p-5">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <h2 className="text-xl font-bold text-foreground dark:text-white">Rendelések ({orders.length})</h2>
              </div>
              <OrdersTable
