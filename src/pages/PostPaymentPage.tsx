...
const CLOSED_ORDER_STATUS_KEYS: string[] = ['atadva', 'torolve'];
...
const PostPaymentPage = () => {
	...
	const resolveOrdersFromEvent = useCallback(
		async (event: OrderRealtimeEvent) => {
			const directOrders = (
				event.orders ?? (event.order ? [event.order] : [])
			).filter((order): order is OrderModel =>
				isOpenOrder(order as OrderModel),
			);
	...
	useEffect(() => {
		...
		const handleSubscriptionError = (status: number) => {
			console.error(
				`Websocket subscription failed on private-${channelName}:`,
				status,
			);
