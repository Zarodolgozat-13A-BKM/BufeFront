...
const compareBySortKey = (
	...
	switch (key) {
		...
		case 'username':
			return a.user_username.localeCompare(b.user_username, 'hu-HU', {
				sensitivity: 'base',
			});
		case 'pickup-time': {
		...
		default:
			return (
				toTimestamp(a.created_at ?? a.delivery_date) -
				toTimestamp(b.created_at ?? b.delivery_date)
			);
	}
};
...
export const AdminOrdersPage = () => {
	...
	const [latestAlertOrderId, setLatestAlertOrderId] = useState<number | null>(
		null,
	);
	...
	useEffect(() => {
		...
		GetAllActiveOrders()
			.then((initialOrders) => {
				if (!isDisposed) {
					setOrders(
						initialOrders.filter((order) => !isClosedOrder(order.status)),
					);
				}
			})
			.catch((error) => {
		...
		const unsubscribe = subscribeToOrderUpdates({
			...
			onSubscriptionError: (status) => {
				console.error(
					'Echo subscription error on private-orders_admin:',
					status,
				);
			},
		});
						...
						<div className='flex flex-wrap items-start justify-between gap-3 mb-4'>
							...
							<div className='flex flex-wrap items-center justify-end gap-2'>
								<div className='flex items-center gap-2 rounded-full border border-[#e6e0db] bg-white px-2 py-1 dark:border-zinc-700 dark:bg-zinc-800'>
									<span className='hidden text-[11px] font-semibold text-muted dark:text-zinc-400 md:block'>
										Rendezés
									</span>
									<select
										value={sortKey}
										...
										<option
											className='bg-white text-foreground dark:bg-zinc-900 dark:text-zinc-100'
											value='created-at'>
											Létrehozás ideje
										</option>
										<option
						...
						{isLoadingOrders ? (
							...
							<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5'>
								{sortedOrders.map((order) =>
									...
									order.status.trim().toLocaleLowerCase('hu-HU') ===
										'fizetésre vár' ? null : (
										<OrderComponent
											key={order.id}
											order={order}
